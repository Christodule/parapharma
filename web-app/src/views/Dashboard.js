...
const Dashboard = () => {
    ...
    useEffect(() => {
        if (usersdata.users) {
            const drivers = usersdata.users.filter(user => user.usertype === 'driver' && ((user.fleetadmin === auth.profile.uid && auth.profile.usertype === 'fleetadmin') || auth.profile.usertype === 'admin'));
            let locs = [];
    ...
    //   const bounds = new window.google.maps.LatLngBounds(mylocation);
