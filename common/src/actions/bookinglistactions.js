...
export const updateBooking = (booking) => async (dispatch) => {
  ...
  if (booking.status == 'NEW' || booking.status == 'ACCEPTED') {
    ...
    if(booking.payment_mode === 'wallet' && parseFloat(booking.usedWalletMoney)>0){
      singleUserRef(booking.customer).once('value', snapshot => {
        let walletBalance = parseFloat(snapshot.val().walletBalance);
        walletBalance = walletBalance - parseFloat(booking.usedWalletMoney);
        singleUserRef(booking.customer).update({"walletBalance": parseFloat(walletBalance.toFixed(settings.decimal))});
        let details = {
          type: 'Debit',
          amount: parseFloat(booking.usedWalletMoney).toFixed(settings.decimal),
          date: new Date().getTime(),
          txRef: booking.id
        }
        walletHistoryRef(booking.customer).push(details);
      });
    }
  }
  ...
  if (booking.status == 'PAID') {
    ...
    if(booking.payment_mode === 'wallet' && !booking.prepaid && parseFloat(booking.usedWalletMoney)>0){
      singleUserRef(booking.customer).once('value', snapshot => {
        let walletBalance = parseFloat(snapshot.val().walletBalance);
        walletBalance = walletBalance - parseFloat(booking.usedWalletMoney);
        singleUserRef(booking.customer).update({"walletBalance": parseFloat(walletBalance.toFixed(settings.decimal))});
