...
const otpCheck = require('regularusedfunctions').otpCheck;
                    ...
                    admin.database().ref("settings").once("value", async settingsdata => {
                        ...
                        for (let dkey in drivers) {
                            ...
                            admin.database().ref("locations/" + dkey).once("value", driverlocdata => {
                                ...
                                if (driver.usertype === 'driver' && driver.approved === true && driver.driverActiveStatus === true && location && driver.licenseImage && driver.carApproved ===true) {
...
exports.verify_email_otp = functions.https.onRequest(async (request, response) => {
    ...
    const listData = emailList.val();
    if(listData){
        let check = await otpCheck(config.firebaseProjectId, email, listData);
        if(check.errorStr){
            await admin.database().ref(`/email_auth_requests/${check.key}`).remove();
            ...
            if(check.data.email){
                if(parseInt(check.data.otp) === parseInt(otp)){
                    ...
                    await admin.database().ref(`/email_auth_requests/${check.key}`).update(check.data);
...
exports.update_auth_email = functions.https.onRequest(async (request, response) => {
    let settingdata = await admin.database().ref('settings').once("value");
    let settings = settingdata.val();
    const allowedOrigins = ['https://' + config.firebaseProjectId + '.web.app', settings.CompanyWebsite];
    const origin = request.headers.origin;
