...
import { MAIN_COLOR, checkCat } from '../common/sharedFunctions';
    ...
    if (payment_mode == 'cash'){
      ...
      setTimeout(()=>{
        if(profile.usertype == 'customer') {
          ...
          props.navigation.navigate('TabRoot', { screen: 'DriverTrips' });
        }
        setIsLoading(false);
      }, 2000);
      ...
      if(profile.usertype == 'customer') {
       ...
       if(booking.status == "REACHED"){
          if((booking.prepaid || curBooking.booking_from_web) && !checkCat(1) ){
            curBooking.status = 'PAID';
          } else{
            curBooking.status = 'PENDING';
          }
