...
import { StyleSheet, FlatList, View, Text, TouchableOpacity, Alert, Share, Switch, Linking } from "react-native";
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    text: t('ok'), onPress: async () => {
                        let userData = {
                            approved: (auth.profile && auth.profile.approved === true)? true: (settings.driver_approval ? false : true),
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => {
                    if (auth.profile && auth.profile.usertype == "customer" && (item.navigationName == "Cars" || item.navigationName == "MyEarning")) {
                        ...
                        return null;
                    } else if (auth.profile && auth.profile.usertype == "customer" && (item.navigationName == "Emergency") && checkCat(2) ) {
                        return null;
                    } else if (auth.profile && auth.profile.usertype == "customer" && (item.navigationName == "Emergency") && !(settings && settings.panic && settings.panic.length>0) && (checkCat(1) || checkCat(3))) {
                        return null;
                    }
                    else {
                        return (
