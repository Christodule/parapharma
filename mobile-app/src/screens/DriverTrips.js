...
import { Text, View, StyleSheet, Dimensions, FlatList, Modal, TouchableHighlight, Switch, Image, Platform, Linking } from 'react-native';
...
import { ExtraInfo, RateView, checkCat } from '../common/sharedFunctions';
...
export default function DriverTrips(props) {
    ...
    const navEditUser = () => {
        props.navigation.dispatch(CommonActions.reset({index: 0, routes:[{ name: 'editUser', params: { fromPage: 'DriverTrips', tabindex: 1} }]}));
    }
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={{ height: height, width: width }}>
                        ...
                        {gps.error || !checks.carExists || !checks.carApproved || !checks.licenseAvailable || !checks.approved || !checks.driverActiveStatus || (settings && settings.imageIdApproval && !checks.imageIdApproval)?
                            <View style={{
                                ...
                                height: 10 +(gps.error ? 70 : 0) + (!checks.driverActiveStatus ? 70 : 0) + (!checks.carExists ? 70 : 0) + (!checks.carApproved ? 70 : 0) + (!checks.approved ? 70 : 0) + (!checks.licenseAvailable ? 70 : 0) + (settings && settings.imageIdApproval && !checks.imageIdApproval ? 70 : 0)
                            }}>
                                {gps.error?
                                        ...
                                        <Button onPress={changePermission}
                                            title={t('fix')} titleStyle={styles.checkButtonTitle} buttonStyle={styles.checkButtonStyle} />
                                    </View>
                                    : null}
                                {!checks.carExists ?
                                          <View style={[styles.alrt,{flexDirection:isRTL?'row-reverse':'row'}]}>
                            ...
                            <View style={styles.mapDetails}>
                                ...
                                {activeBookings && activeBookings.length >= 1 ?
                                    ...
                                    <View style={[styles.detailsBtnView, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                                        ...
                                        {!checkCat(3)|| (checkCat(3) && !(item.driverOffers && item.driverOffers[auth.profile.uid])) ?
