        ...
        if (activeBookings && activeBookings.length >= 1) {
            ...
            if (booking) {
                ...
                if (booking.status == 'NEW' && (booking.bookLater == false || (booking.bookLater && diffMins <= 15))) {
                    if (role == 'customer') setSearchModalVisible(true);
                    if (role == 'customer' && checkCat(3) && booking.selectedBid && !booking.customer_paid){
                        setTimeout(()=>{
            ...
            (curBooking && role == 'customer' && (curBooking.status == 'NEW' || curBooking.status == 'ACCEPTED')) ||
                ...
                <View style={{ flex: 1.5, flexDirection:isRTL? 'row-reverse':'row' }}>
                    {(role == 'customer' && !curBooking.pickup_image && (curBooking.status == 'NEW' || curBooking.status == 'ACCEPTED')) ||
                        ...
                        : null}
                    {checkCat(2) && settings.AllowDeliveryPickupImageCapture && role == 'driver' && !curBooking.pickup_image && (curBooking.status == 'ACCEPTED' || curBooking.status == 'ARRIVED') ?
                        <View style={{ flex: 1 }}>
                            <Button
                        ...
                        : null}
                    {role == 'driver' && (checkCat(3)  || (curBooking.pickup_image && checkCat(2)) || (!settings.AllowDeliveryPickupImageCapture && checkCat(2)) || checkCat(1)) && (curBooking.status == 'ACCEPTED' || curBooking.status == 'ARRIVED') ?
                        <View style={{ flex: 1 }}>
                            <Button
                                title={t('start_trip')}
                                loading={false}
                                ...
                                titleStyle={{ color: colors.WHITE, fontWeight: 'bold' }}
                                onPress={() => {
                                    if (curBooking.otp && (checkCat(1) || checkCat(3))) {
                    ...
                    {checkCat(2) && settings.AllowFinalDeliveryImageCapture && role == 'driver' && !curBooking.deliver_image && curBooking.status == 'STARTED' ?
                        <View style={{ flex: 1 }}>
                            <Button
                        ...
                        : null}
                    {role == 'driver' && (checkCat(3) || (curBooking.deliver_image && checkCat(2)) || (!settings.AllowFinalDeliveryImageCapture && checkCat(2)) || checkCat(1)) && curBooking.status == 'STARTED' ?
                        <View style={{ flex: 1 }}>
                            <Button
            ...
            >
               <View style={{ flex: 1, backgroundColor: colors.BACKGROUND, justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    <View style={{ width: width - 40, backgroundColor: colors.WHITE, borderRadius: 10, flex: 1, maxHeight: height - 200,marginTop:15 }}>
                        ...
                        <View style={{marginTop: 60,width: width - 60, height: height - 340, marginRight:10,marginLeft:10, alignSelf:'center'}}>
                            <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1}}>
                                {Object.keys(curBooking.driverOffers).map(key=>  
                                    <View key={key} style={{height: 125, marginBottom: 20,borderColor: 'black', borderWidth: 2,borderRadius: 10,}}>
                                        <View style={{marginTop:8, marginBottom:8, flexDirection:isRTL?'row-reverse':'row',width:'100%',justifyContent:'space-evenly',padding:2,height:50}} >
                                            <View style={{flexDirection:isRTL?'row-reverse':'row',width:'68%',height:50}}>
                                                ...
                                                <View style={{width:'75%'}}><Text style={{ color: colors.BLACK, fontSize: 16,marginTop:8,textAlign:'center',}}>{curBooking.driverOffers[key].driver_name}</Text>
                                                 <Text style={{ color: colors.BLACK, fontSize: 22, fontWeight: '700',alignSelf:'center'}}>{settings.symbol} {curBooking.driverOffers[key].trip_cost}</Text>
                                                </View>
                                                 </View>
                                                    <View style={{width:'30%'}}>
                                                        <StarRating
                                                        ...
                                                        <Button
                                                            ...
                                                            buttonStyle={{width:90,marginTop:8, backgroundColor: colors.GREEN_DOT}}
                                                        />
                                                    </View>
                                        </View>
                                        <Text style={{ color: colors.BLACK, fontSize: 16,fontWeight: '600',marginTop:10,alignSelf:'center'}}>{moment(curBooking.driverOffers[key].deliveryDate).format('lll')}</Text>
    ...
    return (
        <View style={styles.mainContainer}>
            <View style={styles.mapcontainer}>
                ...
                {curBooking && !(curBooking.status == 'NEW') ?
                    <TouchableOpacity
                        ...
                        onPress={() => role == 'customer' ? onPressCall(curBooking.driver_contact) : ((checkCat(1) || checkCat(3)) ? onPressCall(curBooking.customer_contact) : setUserInfoModalStatus(true))}
                    >
                        <Icon
                            name="ios-call"
                            type="ionicon"
            ...
            <OtpModal
                ...
                onMatch={(value) => value ? (checkCat(1) || checkCat(3)) ? startBooking() : endBooking() : null}
            />
        </View>
    );
