...
export default function MapScreen(props) {
    ...
    const onPressBook = async () => {
        if ((settings && settings.imageIdApproval && auth.profile.verifyId && auth.profile.verifyIdImage) || (settings && !settings.imageIdApproval)) {
            if (parseFloat(profile.walletBalance) >= 0) {
                ...
                if (!(profile.mobile && profile.mobile.length > 6)) {
                    ...
                    props.navigation.dispatch(CommonActions.reset({ index: 0, routes: [{ name: 'editUser', params: { fromPage: 'Map', tabindex: 0  } }] }));
                    setBookLoading(false);
       ...
       Alert.alert(
                ...
                [
                    ...
                    { text: t('ok'), onPress: () =>   
                        props.navigation.dispatch(CommonActions.reset({index: 0, routes:[{ name: 'editUser', params: { fromPage: 'Map', tabindex: 1 } }]})) 
                    }
                ],
                { cancelable: false }
            );
    ...
    const onPressBookLater = () => {
        setCheckType(false);
        if ((settings && settings.imageIdApproval && auth.profile.verifyId && auth.profile.verifyIdverifyIdImage) || (settings && !settings.imageIdApproval)) {
            if (parseFloat(profile.walletBalance) >= 0) {
                if (!(profile.mobile && profile.mobile.length > 6)) {
                    Alert.alert(t('alert'), t('mobile_need_update'));
                    props.navigation.dispatch(CommonActions.reset({ index: 0, routes: [{ name: 'editUser', params: { fromPage: 'Map', tabindex: 0 } }] }));
            ...
            Alert.alert(
                ...
                [
                    { text: t('cancel'), onPress: () => { }, style: 'cancel' },
                    { text: t('ok'), onPress: () =>   
                        props.navigation.dispatch(CommonActions.reset({index: 0, routes:[{ name: 'editUser', params: { fromPage: 'MapScreen', tabindex: 1} }]})) 
                    }
                ],
                { cancelable: false }
            );
    ...
    const bookNow = async () => {
        ...
        if ((radioProps[payment_mode].cat === 'wallet' && notfound) || radioProps[payment_mode].cat !== 'wallet') {
            if ((radioProps[payment_mode].cat === 'wallet' && (parseFloat(wallet_balance) >= parseFloat(estimatedata.estimate.estimateFare)) && !checkCat(3)) || radioProps[payment_mode].cat !== 'wallet' || (radioProps[payment_mode].cat === 'wallet' && checkCat(3))) {
                const addBookingObj = {
